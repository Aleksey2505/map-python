# 🚁 Aeroscope Vision - Интеллектуальный анализатор аэросъемки

## 🌟 О проекте

![Интерфейс программы](interface.png)
![пример логов программы](log.png)

**Aeroscope Vision** - это профессиональное desktop-приложение для автоматизированного анализа аэрофотосъемки и видео с дронов. Система объединяет передовые технологии компьютерного зрения, геопозиционирования и картографии для обнаружения людей и объектов в воздушной разведке.

## 🎯 Ключевые возможности

### 👁️ Компьютерное зрение премиум-класса
- **YOLO-детекция** людей на фото и видео с дронов
- **Автоматическое выделение** bounding boxes с координатами
- **Настраиваемые параметры** точности (confidence, IoU)
- **Пакетная обработка** больших объемов данных

### 🗺️ Интегрированная геоаналитика
- **Парсинг GPS-меток** из SRT-субтитров видеозаписей
- **Чтение EXIF-данных** с координатами из фотографий
- **Создание интерактивных карт** с кластеризацией маркеров
- **Визуализация на спутниковых снимках** Esri World Imagery

### 📊 Профессиональная отчетность
- **Генерация Excel-отчетов** с полной метаинформацией
- **Логирование всех операций** в текстовые файлы
- **Статистика обнаружений** по времени и координатам
- **Экспорт в HTML-карты** с превью изображений

## 🛠 Технологический стек

### 🤖 Искусственный интеллект
- **Ultralytics YOLO** - современные нейросетевые модели
- **OpenCV** - обработка изображений и видео
- **NumPy** - математические вычисления

### 🗄️ Обработка данных
- **Pandas** - анализ и экспорт табличных данных
- **OpenPyXL** - работа с Excel-файлами
- **Base64** - кодирование изображений для веб

### 🗺️ Геопозиционирование
- **Folium + MarkerCluster** - интерактивные карты
- **Piexif** - работа с EXIF-метаданными
- **SRT-парсер** - извлечение координат из субтитров

### 🎨 Графический интерфейс
- **PySide6** - современный Qt-интерфейс
- **QSettings** - сохранение пользовательских настроек
- **Стилизованный дизайн** - темно-синяя тема

## 🎮 Режимы работы

### 🎥 Видеоаналитика
- **Обработка MP4/AVI/MOV** файлов с дронов
- **Синхронизация с SRT** для точного позиционирования
- **Извлечение ключевых кадров** с обнаруженными объектами
- **Временная привязка** каждого обнаружения

### 📸 Фотоанализ
- **Пакетная обработка** папок с аэрофото
- **Автоопределение GPS** из EXIF-данных
- **Фильтрация по наличию** людей/объектов
- **Гибкие настройки** отображения на картах

## 📈 Выходные данные

### 🗂️ Структурированные отчеты
- **Excel-таблицы** с координатами и временными метками
- **Визуализированные карты** с кластерами обнаружений
- **Аннотированные изображения** с bounding boxes
- **Детальные логи** процесса обработки

### 🌐 Интерактивные карты
- **Маркеры с превью** - при клике отображается изображение
- **Кластеризация** для плотных скоплений точек
- **Спутниковые подложки** Esri World Imagery
- **Информационные всплывающие окна** с деталями

## 🎯 Целевая аудитория

### 🚁 Операторы БПЛА
- **Анализ миссий** аэрофотосъемки
- **Поисково-спасательные** операции
- **Мониторинг территорий** и объектов

### 🏢 Промышленные компании
- **Контроль безопасности** периметров
- **Инвентаризация активов** с воздуха
- **Мониторинг строительных** площадок

### 🛡️ Службы безопасности
- **Патрулирование границ** и зон отдыха
- **Поиск пропавших** в труднодоступных местах
- **Наблюдение за массовыми** мероприятиями

### 🔬 Научные исследования
- **Экологический мониторинг**
- **Археологические разведки**
- **Изучение миграций** животных и людей

## ⚡ Преимущества системы

### 🚀 Высокая производительность
- **Оптимизированная обработка** больших datasets
- **Многопоточность** для пакетных операций
- **Минимальные требования** к hardware

### 🎯 Точность и надежность
- **Современные AI-модели** с высокой детекцией
- **Геометрическая привязка** с субсантиметровой точностью
- **Валидация данных** на всех этапах обработки

### 🔧 Простота использования
- **Интуитивный интерфейс** с темной темой
- **Автосохранение настроек** между сессиями
- **Подробное логирование** и статус выполнения

## 📋 Системные требования

- **ОС**: Windows 10/11, Linux, macOS
- **Python**: 3.8+
- **Память**: 4GB+ RAM
- **GPU**: Опционально (ускоряет YOLO-обработку)
- **Интернет**: Для загрузки картографических подложек

## 🚀 Быстрый старт

1. **Выберите режим** - видео или фото обработка
2. **Укажите модель YOLO** для детекции объектов
3. **Загрузите исходные данные** - видео+SRT или папку с фото
4. **Настройте параметры** детекции и фильтрации
5. **Запустите обработку** и получите комплексный отчет












# Блок-схема приложения детекции людей



```mermaid
flowchart TD
    E0[Этап 0: Идея] --> E1
    E1[Этап 1: Прототип] --> E2
    E2[Этап 2: Геотеги] --> E3
    E3[Этап 3: Карта] --> E4
    E4[Этап 4: Режим фото] --> E5
    E5[Этап 5: GUI] --> E6
    E6[Этап 6: Отчеты] --> E7[Этап 7: Финальные штрихи]
    
    E1 --> M1[SRTParser + cv2 + YOLO]
    E2 --> M2[dms_to_exif + piexif]
    E3 --> M3[folium + MarkerCluster]
    E4 --> M4[get_gps_from_exif]
    E5 --> M5[PySide6 + QSettings]
    E6 --> M6[pandas + logging]
    E7 --> M7[draw_bounding_boxes]
```



```mermaid
flowchart TD
    A[Запуск приложения PySide6 + QSettings] --> B{Режим работы?}
    
    B --> C1[Видео + SRT]
    B --> C2[Папка с фото]
    
    C1 --> V[Открыть видео cv2 + SRTParser]
    C2 --> P[Поиск фото glob + piexif GPS]
    
    V --> E[YOLO детекция conf/iou class 0 = человек]
    P --> E
    
    E --> F{Найден человек?}
    F -->|Да| G[Рисуем боксы save_frame_with_gps + piexif GPS]
    F -->|Нет| H[Пропустить или на карту если чекбокс]
    
    G --> I[Сохраняем результаты в data для Excel в map_data для карты]
    H --> I
    
    I --> J[Прогресс-бар QApplication.processEvents]
    J --> K{Конец цикла?}
    K -->|Нет| E
    K -->|Да| L[Генерация отчётов]
    
    L --> M{Есть кадры с людьми?}
    M -->|Да| N[Excel: frames_metadata.xlsx автопереименование при ошибке]
    M -->|Нет| O[Excel не создан]
    
    L --> Q{Есть точки для карты?}
    Q -->|Да| R[folium карта Esri.WorldImagery MarkerCluster + base64 превью]
    Q -->|Нет| S[Карта не создана]
    
    R --> T[Открыть в браузере]
    N --> U[status_label: ГОТОВО!]
    R --> U
    
    style A fill:#1e40af,color:white
    style E fill:#dc2626,color:white
    style R fill:#059669,color:white
    style U fill:#7c3aed,color:white,font-weight:bold
```


# Используемые инструменты и библиотеки

![Библиотеки Python](lib.png)

## 🐍 Системные и базовые библиотеки

### `import sys` - работа с системными параметрами Python
- Доступ к аргументам командной строки
- Управление путями импорта модулей  
- Взаимодействие со стандартными потоками ввода/вывода/ошибок
- Выход из программы с определенным кодом
- Получение информации о версии Python и платформе

**Пример использования:**
![Пример sys](sys.png)

### `import os` - взаимодействие с операционной системой
- Работа с файловой системой (создание, удаление, перемещение файлов)
- Доступ к переменным окружения
- Управление процессами и получение информации о системе
- Работа с путями файлов (абсолютные/относительные пути)
- Выполнение системных команд

**Пример использования:**
![Пример os](os.png)

### `import base64` - кодирование и декодирование данных
- Преобразование бинарных данных в текстовый формат ASCII
- Кодирование изображений, файлов для передачи по текстовым протоколам
- Создание Data URL для веб-страниц
- Шифрование простых данных (базовое)

## 🎨 Графические интерфейсы (GUI)

### `from PySide6.QtWidgets import ...` - создание desktop приложений
- Окна, кнопки, поля ввода, метки, выпадающие списки
- Размещение элементов через layouts (вертикальные, горизонтальные)
- Диалоговые окна (выбор файлов, сообщения)
- Прогресс-бары, радио-кнопки, чекбоксы
- Текстовые редакторы и сложные интерфейсы

### `from PySide6.QtCore import ...` - базовые функции Qt
- Настройки приложения (QSettings)
- Многопоточность и таймеры
- Обработка событий и сигналов
- Работа с датами, временем, строками

**Примеры интерфейсов:**
![Интерфейс PySide6](PySide61.png)
![Интерфейс PySide6](PYSIDE6.jpg)

## 🖼️ Обработка изображений и компьютерное зрение

### `import cv2 (OpenCV)` - компьютерное зрение
- Загрузка, сохранение и отображение изображений/видео
- Обнаружение и распознавание объектов
- Обработка изображений (фильтры, преобразования)
- Работа с камерами и видео потоками
- Машинное зрение и анализ изображений

**Пример использования cv2:**
![Запись видео](recording_python_20348.gif)
*Справа создается файл записи. Слева мы видим что идет запись в программе.*

### `import piexif` - работа с метаданными изображений
- Чтение и запись EXIF данных (информация о фотоаппарате, настройках)
- Извлечение GPS координат из фотографий
- Редактирование метаданных JPEG/TIFF файлов
- Удаление приватной информации из изображений

## 🤖 Машинное обучение и AI

### `from ultralytics import YOLO` - современные модели детекции
- Обнаружение объектов на изображениях и видео
- Сегментация экземпляров
- Классификация изображений
- Оценка позы человека
- Высокая скорость и точность обработки

## 📊 Анализ данных

### `import pandas as pd` - анализ табличных данных
- Работа с DataFrame (таблицами данных)
- Фильтрация, сортировка, группировка данных
- Агрегация и статистический анализ
- Чтение/запись различных форматов (CSV, Excel, JSON)
- Обработка временных рядов

### `import numpy as np` - численные вычисления
- Работа с многомерными массивами
- Математические операции над массивами
- Линейная алгебра и статистика
- Интеграция с библиотеками машинного обучения

## 🗺️ Геоданные и карты

### `import folium` - создание интерактивных карт
- Визуализация данных на картах OpenStreetMap
- Добавление маркеров, кругов, линий, полигонов
- Создание тепловых карт и кластеров маркеров
- Экспорт карт в HTML для веб-страниц

### `from folium.plugins import MarkerCluster` - кластеризация маркеров
- Группировка близко расположенных маркеров
- Улучшение читаемости карт с большим количеством точек
- Автоматическое управление отображением маркеров

## 🌐 Веб и автоматизация

### `import webbrowser` - управление веб-браузером
- Автоматическое открытие URL в браузере
- Открытие локальных HTML файлов
- Управление вкладками и окнами браузера
- Интеграция веб-контента в desktop приложения

## ⏰ Работа со временем

### `import time` - базовые временные функции
- Задержки выполнения кода (sleep)
- Измерение времени выполнения операций
- Работа с временными метками

### `from datetime import timedelta` - интервалы времени
- Представление продолжительности времени
- Арифметические операции с датами и временем
- Расчет разницы между временными точками

## 📁 Работа с файлами и данными

### `from glob import glob` - поиск файлов по шаблонам
- Рекурсивный поиск файлов по маскам
- Работа с путями файлов используя wildcards
- Упрощение обработки множества файлов

## 📝 Обработка текста

### `import re` - регулярные выражения
- Поиск и замена текста по сложным шаблонам
- Валидация форматов данных
- Парсинг и извлечение информации из текста
- Сложные операции с текстовыми данными

## 📋 Логирование и отладка

### `import logging` - система логирования
- Запись логов разных уровней (debug, info, warning, error)
- Настройка формата вывода и мест сохранения
- Отслеживание работы приложения
- Диагностика ошибок и проблем

## 📑 Работа с Excel

### `import openpyxl` - работа с Excel файлами
- Чтение и запись .xlsx файлов
- Создание и редактирование листов, ячеек, формул
- Работа со стилями и форматированием
- Автоматизация работы с таблицами Excel

## 🎯 Что можно создать с таким набором

- Desktop приложение для анализа изображений с AI-детекцией объектов
- Систему обработки геоданных с визуализацией на картах
- Инструмент анализа данных с экспортом в Excel и графиками
- Автоматизированную систему для обработки фотографий и их метаданных
- GUI оболочку для моделей машинного обучения
- Комплексное решение для компьютерного зрения с интерфейсом и отчетностью




```mermaid
flowchart TD
    A[Пользователь нажимает кнопку] --> B{Режим?}
    B -->|Видео| C[Открыть MP4 + SRT]
    B -->|Фото| D[Сканировать папку]
    
    C --> E[Цикл по кадрам с шагом N]
    D --> F[Цикл по всем JPG/PNG]
    
    E --> G[Получить GPS из SRT]
    F --> H[Получить GPS из EXIF]
    
    G --> I[Извлечь кадр из видео]
    H --> J[Прочитать фото]
    
    I --> K[YOLO детекция]
    J --> K
    
    K --> L{Люди найдены?}
    L -->|Да| M[Рисовать bbox]
    L -->|Нет| N[Пропустить для Excel]
    
    M --> O[Сохранить с GPS в EXIF]
    O --> P[Добавить в data + map_data]
    N --> Q{Показывать все на карте?}
    Q -->|Да| R[Добавить только в map_data]
    Q -->|Нет| S[Пропустить]
    
    P --> T[Создать Excel]
    R --> U[Создать Folium карту]
    S --> U
    
    T --> V[Открыть карту в браузере]
    U --> V
```




# Полный анализ функций проекта

## Класс `SRTParser`

| Функция | Параметры | Возвращает | Назначение | Взаимосвязь с проектом |
|---------|-----------|------------|------------|------------------------|
| `__init__` | `srt_path: str` | - | Инициализирует парсер и сразу парсит SRT-файл | Создаёт объект с готовым списком субтитров для дальнейшей работы |
| `parse_srt` | `srt_path: str` | `list[dict]` | Читает и структурирует SRT-файл в список словарей | Обрабатывает две кодировки (utf-8, latin-1), разбивает на блоки: индекс, временные метки, текст телеметрии |
| `str_to_time` | `time_str: str` | `timedelta` | Конвертирует строку времени "HH:MM:SS,mmm" в объект timedelta | Преобразует временные метки из SRT в формат, с которым можно работать математически |
| `get_gps_at_frame` | `frame_index: int` | `tuple(lat, lon, time, alt)` | Извлекает GPS-координаты и высоту для конкретного кадра | Ищет в тексте субтитра паттерны `[latitude: ...]`, `[longtitude: ...]`, `[rel_alt: ...]` и возвращает числовые значения |

---

## Класс `VideoFrameExtractor` (главное окно)

### Инициализация и UI

| Функция | Параметры | Возвращает | Назначение | Взаимосвязь с проектом |
|---------|-----------|------------|------------|------------------------|
| `__init__` | - | - | Создаёт главное окно приложения | Инициализирует `QSettings` для сохранения путей, строит весь UI (кнопки, поля ввода, прогресс-бар, логи), применяет тёмную тему |
| `browse_video` | - | - | Открывает диалог выбора видео-файла | При выборе файла записывает путь в `video_path_edit` и сохраняет в `QSettings` |
| `browse_srt` | - | - | Открывает диалог выбора SRT-файла | Аналогично `browse_video`, но для телеметрии |
| `browse_photo_folder` | - | - | Открывает диалог выбора папки с фото | Позволяет выбрать директорию для режима обработки фотографий |
| `browse_model` | - | - | Открывает диалог выбора .pt модели YOLO | Пользователь может указать свою обученную модель |

### Работа с изображениями и GPS

| Функция | Параметры | Возвращает | Назначение | Взаимосвязь с проектом |
|---------|-----------|------------|------------|------------------------|
| `dms_to_exif` | `degrees: float` | `list[tuple]` | Конвертирует десятичные градусы в формат EXIF (градусы, минуты, секунды) | Формат EXIF требует GPS в виде `[(град, 1), (мин, 1), (сек, 10000)]` — функция выполняет эту конвертацию |
| `save_frame_with_gps` | `frame: ndarray`<br>`frame_path: str`<br>`lat: float`<br>`lon: float` | `bool` | Сохраняет кадр как JPEG и вшивает GPS-теги в EXIF | **Ключевая функция**: кадры из видео не имеют EXIF → эта функция восстанавливает геотеги из SRT и записывает их в изображение через `piexif` |
| `get_gps_from_exif` | `image_path: str` | `tuple(lat, lon)` | Читает GPS-координаты из EXIF фотографии | Используется в режиме обработки фотографий — извлекает существующие геотеги из JPG/PNG |
| `encode_image_base64` | `image_path: str` | `str` (data URI) | Кодирует изображение в base64 для встраивания в HTML | Позволяет вставить полноразмерное фото прямо в popup карты без внешних ссылок |

### Детекция и визуализация

| Функция | Параметры | Возвращает | Назначение | Взаимосвязь с проектом |
|---------|-----------|------------|------------|------------------------|
| `draw_bounding_boxes` | `frame: ndarray`<br>`results: list` | `ndarray` | Рисует красные прямоугольники вокруг обнаруженных людей | Визуализирует результаты YOLO — рисует bbox и показывает confidence score над каждым человеком |
| `format_time` | `seconds: float` | `str` (HH:MM:SS) | Форматирует секунды в читаемый формат времени | Используется для tooltip на карте — вместо "125.5" показывает "00:02:05" |

### Основная обработка

| Функция | Параметры | Возвращает | Назначение | Взаимосвязь с проектом |
|---------|-----------|------------|------------|------------------------|
| `extract_frames_or_photos` | - | - | **Главная функция** обработки видео или папки с фото | **Оркестрирует весь процесс**: <br>1. Проверяет входные данные<br>2. Создаёт output директорию<br>3. Загружает YOLO модель<br>4. В режиме видео: открывает MP4, парсит SRT, для каждого N-го кадра извлекает GPS → запускает детекцию → если найдены люди, рисует bbox и сохраняет с GPS<br>5. В режиме фото: читает все JPG/PNG, извлекает GPS из EXIF → запускает детекцию → сохраняет только фото с людьми<br>6. Формирует два набора данных: `data` (только с людьми для Excel) и `map_data` (для карты, может включать все фото)<br>7. Создаёт Excel-файл с метаданными<br>8. Создаёт интерактивную Folium-карту с кластеризацией и base64-превью<br>9. Открывает карту в браузере<br>10. Обновляет прогресс-бар и логи на каждом шаге |

---

## Служебные функции

| Функция | Параметры | Возвращает | Назначение | Взаимосвязь с проектом |
|---------|-----------|------------|------------|------------------------|
| `setup_logging` | `output_dir: str` | - | Настраивает запись логов в файл `log.txt` | Вызывается перед началом обработки — все последующие `logging.info()` пишутся и в файл, и дублируются в GUI |

---

## Архитектура данных

### Структура `data` (для Excel)
```python
{
    'Frame Number': int,        # Номер кадра/фото
    'Time (seconds)': float,    # Время в видео
    'Latitude': float,          # Широта
    'Longitude': float,         # Долгота
    'Altitude': float/None,     # Высота (только для видео)
    'Person Count': int,        # Количество людей
    'Frame Path': str           # Абсолютный путь к сохранённому кадру
}
```
**Содержит только кадры/фото с обнаруженными людьми**

### Структура `map_data` (для карты)
Аналогична `data`, но может включать **все** фото (если включён чекбокс "Отображать все фото на карте")

---

## Поток выполнения

```mermaid
graph TD
    A[Пользователь нажимает кнопку] --> B{Режим?}
    B -->|Видео| C[Открыть MP4 + SRT]
    B -->|Фото| D[Сканировать папку]
    
    C --> E[Цикл по кадрам с шагом N]
    D --> F[Цикл по всем JPG/PNG]
    
    E --> G[Получить GPS из SRT]
    F --> H[Получить GPS из EXIF]
    
    G --> I[Извлечь кадр из видео]
    H --> J[Прочитать фото]
    
    I --> K[YOLO детекция]
    J --> K
    
    K --> L{Люди найдены?}
    L -->|Да| M[Рисовать bbox]
    L -->|Нет| N[Пропустить для Excel]
    
    M --> O[Сохранить с GPS в EXIF]
    O --> P[Добавить в data + map_data]
    N --> Q{Показывать все на карте?}
    Q -->|Да| R[Добавить только в map_data]
    Q -->|Нет| S[Пропустить]
    
    P --> T[Создать Excel]
    R --> U[Создать Folium карту]
    S --> U
    
    T --> V[Открыть карту в браузере]
    U --> V
```

---

## Критические зависимости между функциями

1. **`parse_srt` → `get_gps_at_frame` → `save_frame_with_gps`**
   - SRT парсится один раз при инициализации
   - Для каждого кадра извлекаются GPS-координаты
   - Эти координаты вшиваются в JPEG через EXIF

2. **`dms_to_exif` ↔ `save_frame_with_gps`**
   - Конвертация формата необходима, т.к. EXIF требует специфическую структуру данных

3. **`draw_bounding_boxes` → `save_frame_with_gps`**
   - Сначала на кадр накладываются визуализации детекции
   - Затем изображение сохраняется с GPS

4. **`encode_image_base64` → Folium карта**
   - Каждое изображение кодируется в строку
   - Эта строка вставляется в HTML popup маркера

5. **`extract_frames_or_photos` координирует все**
   - Вызывает все остальные функции в правильном порядке
   - Управляет двумя потоками данных (Excel и карта)
   - Обеспечивает обновление UI (прогресс-бар, логи)

---

## Особенности реализации

| Аспект | Решение | Причина |
|--------|---------|---------|
| Два режима работы | `QRadioButton` переключает логику | Один инструмент для видео И фотографий |
| Две кодировки в SRT | `try/except` с fallback на latin-1 | Разные дроны используют разные кодировки |
| Две структуры данных | `data` (только люди) и `map_data` (может быть все) | Excel показывает только важное, карта может показывать контекст |
| Прогресс-бар + `processEvents()` | Явное обновление UI в цикле | Без этого окно "зависнет" на десятках тысяч кадров |
| Base64 в карте | Встраивание вместо ссылок на файлы | Карта работает даже если переместить HTML-файл |
| `os.path.normpath()` везде | Нормализация путей | Кроссплатформенность (Windows `\` vs Unix `/`) |
| Regex для GPS | Гибкий поиск паттернов | Формат телеметрии может немного отличаться |
| Обработка `PermissionError` | Автотаймстамп при конфликте | Если Excel открыт, создаёт новый файл |
| `QSettings` | Автосохранение путей | UX: при повторном запуске всё заполнено |

# 🎯 Архитектура и эволюция проекта

## 📊 Блок-схема выполнения программы

```mermaid
graph TD
    Start[👤 Пользователь нажимает 'Извлечь кадры...'] --> Mode{🔀 Выбор режима}
    
    Mode -->|📹 Режим ВИДЕО| VideoOpen[📂 Открытие видео<br/>cv2.VideoCapture]
    Mode -->|📸 Режим ФОТО| PhotoScan[📁 Сканирование папки<br/>glob *.jpg + *.png]
    
    VideoOpen --> VideoInfo[📊 Определение FPS<br/>и общего кол-ва кадров]
    VideoOpen --> SRTOpen[📄 Открытие SRT<br/>SRTParser]
    
    SRTOpen --> SRTParse[🔍 Парсинг всех субтитров<br/>с GPS-данными]
    
    VideoInfo --> LoopFrames[🔄 ЦИКЛ ПО КАДРАМ<br/>шаг: 1/5/10/20]
    SRTParse --> LoopFrames
    
    LoopFrames --> SetFrame[⏩ cap.set CAP_PROP_POS_FRAMES]
    SetFrame --> ReadFrame[📖 ret, frame = cap.read]
    
    PhotoScan --> LoopPhotos[🔄 ЦИКЛ ПО ФОТОГРАФИЯМ]
    LoopPhotos --> GetGPS[🌍 get_gps_from_exif<br/>Чтение GPS из EXIF]
    GetGPS --> ReadPhoto[📖 cv2.imread]
    
    ReadFrame --> YOLOInference[🤖 YOLO Детекция<br/>model frame, conf, iou]
    ReadPhoto --> YOLOInference
    
    YOLOInference --> CheckPerson{👥 Найден класс 0<br/>person?}
    
    CheckPerson -->|❌ Нет людей| Skip[⏭️ Пропустить кадр]
    CheckPerson -->|✅ Люди найдены| DrawBoxes[🎨 draw_bounding_boxes<br/>Рисование bbox + conf]
    
    DrawBoxes --> SaveGPS[💾 save_frame_with_gps<br/>+ piexif.insert]
    SaveGPS --> AddData[📝 Добавление в:<br/>• data список<br/>• map_data список]
    
    Skip --> CheckAll{🗺️ Показывать все<br/>на карте?}
    CheckAll -->|Да| AddMapOnly[📍 Добавить только в map_data]
    CheckAll -->|Нет| Continue
    
    AddData --> Progress[📊 Обновление<br/>QProgressBar + processEvents]
    AddMapOnly --> Progress
    
    Progress --> LoopCheck{🔄 Есть ещё<br/>кадры/фото?}
    LoopCheck -->|Да| LoopFrames
    LoopCheck -->|Да| LoopPhotos
    LoopCheck -->|Нет| Release[🔒 cap.release<br/>Закрытие видео]
    
    Release --> CreateExcel[📊 СОЗДАНИЕ EXCEL<br/>pandas.DataFrame data]
    Continue --> CreateExcel
    
    CreateExcel --> ExcelSave[💾 df.to_excel<br/>с обработкой PermissionError]
    ExcelSave --> CreateMap[🗺️ СОЗДАНИЕ КАРТЫ<br/>pandas.DataFrame map_data]
    
    CreateMap --> MapCenter[📍 Расчёт центра карты<br/>avg lat, lon]
    MapCenter --> FoliumInit[🌍 folium.Map<br/>Esri.WorldImagery + MarkerCluster]
    
    FoliumInit --> LoopMarkers[🔄 Для каждой точки:]
    LoopMarkers --> EncodeB64[🖼️ encode_image_base64<br/>Кодирование превью]
    EncodeB64 --> CreatePopup[💬 Создание popup<br/>img src data:image/jpeg]
    CreatePopup --> CreateTooltip[ℹ️ Создание tooltip<br/>Frame, Time, Lat, Lon, Alt]
    CreateTooltip --> AddMarker[📌 folium.Marker.add_to<br/>marker_cluster]
    
    AddMarker --> MoreMarkers{🔄 Есть ещё<br/>точки?}
    MoreMarkers -->|Да| LoopMarkers
    MoreMarkers -->|Нет| SaveMap[💾 m.save frames_map.html]
    
    SaveMap --> OpenBrowser[🌐 webbrowser.open<br/>Открытие карты]
    OpenBrowser --> UpdateUI[✅ Обновление status_label<br/>и логов]
    UpdateUI --> End[🎉 ГОТОВО!]

    style Start fill:#4CAF50,stroke:#2E7D32,color:#fff
    style End fill:#4CAF50,stroke:#2E7D32,color:#fff
    style YOLOInference fill:#FF9800,stroke:#F57C00,color:#fff
    style SaveGPS fill:#2196F3,stroke:#1565C0,color:#fff
    style CreateMap fill:#9C27B0,stroke:#6A1B9A,color:#fff
    style CreateExcel fill:#009688,stroke:#00695C,color:#fff
```

---

## 🚀 История разработки проекта

### 🌱 **Этап 0: Идея**
> **Концепция:** Нужно искать людей на дроне-видео и автоматически ставить их на карту

---

### 🔧 **Этап 1: Базовый прототип (Видео + SRT)**

**Задача:** Создать минимально рабочую версию для видео

**Реализовано:**
- ✅ **`SRTParser`** (все 4 метода)
  - Парсинг телеметрии дронов
  - Извлечение GPS по номеру кадра
- ✅ **OpenCV** (`cv2`)
  - Чтение видео
  - Навигация по кадрам
- ✅ **YOLO детекция**
  - Поиск людей на кадрах
- ✅ **Консольный вывод**
  - Сохранение кадров с людьми
  - Печать координат в терминал

**Результат:** Работающий скрипт, который находит людей и показывает их координаты

---

### 🌍 **Этап 2: Геотеги**

**Задача:** Сделать кадры GIS-совместимыми

**Реализовано:**
- ✅ **`dms_to_exif()`**
  - Конвертация десятичных градусов в формат EXIF
- ✅ **`save_frame_with_gps()`**
  - Встраивание GPS в JPEG через `piexif.insert()`

**Результат:** Кадры можно открыть в любом GIS-приложении (QGIS, ArcGIS, Google Earth)

---

### 🗺️ **Этап 3: Интерактивная карта**

**Задача:** Визуализировать результаты на карте

**Реализовано:**
- ✅ **Folium + MarkerCluster**
  - Интерактивная карта с группировкой маркеров
- ✅ **`encode_image_base64()`**
  - Встраивание превью прямо в HTML
- ✅ **HTML-карта с popup**
  - Кликабельные маркеры с фотографиями

**Результат:** Профессиональная интерактивная карта с Esri спутниковыми снимками

---

### 📸 **Этап 4: Режим обработки фотографий**

**Задача:** Расширить функционал на готовые фотографии

**Реализовано:**
- ✅ **`get_gps_from_exif()`**
  - Чтение существующих GPS-тегов из фото
- ✅ **Ветвление в `extract_frames_or_photos()`**
  - Второй режим работы (фото вместо видео)
- ✅ **Обработка JPG/PNG**
  - Поддержка разных форматов

**Результат:** Один инструмент для двух задач (видео + фото)

---

### 🖥️ **Этап 5: GUI и UX**

**Задача:** Сделать приложение удобным для пользователя

**Реализовано:**
- ✅ **PySide6**
  - Полноценный графический интерфейс
  - Тёмная тема с градиентами
- ✅ **QSettings**
  - Автосохранение путей между запусками
- ✅ **QFileDialog**
  - Удобный выбор файлов и папок
- ✅ **QProgressBar**
  - Визуализация прогресса обработки
- ✅ **Методы `browse_*()`**
  - Кнопки для выбора файлов
- ✅ **Чекбокс "показывать все фото"**
  - Гибкая настройка вывода на карту

**Результат:** Интуитивное приложение с сохранением настроек

---

### 📊 **Этап 6: Отчёты и надёжность**

**Задача:** Добавить отчётность и обработку ошибок

**Реализовано:**
- ✅ **pandas → Excel**
  - Структурированные таблицы с метаданными
- ✅ **logging**
  - Полное логирование в файл + GUI
- ✅ **Обработка `PermissionError`**
  - Автоматическое переименование при конфликте файлов
- ✅ **`processEvents()`**
  - Отзывчивый UI при длительной обработке

**Результат:** Стабильная работа с полным аудит-треилом

---

### ✨ **Этап 7: Финальные штрихи**

**Задача:** Довести до production-качества

**Реализовано:**
- ✅ **`draw_bounding_boxes()`**
  - Визуализация результатов детекции на кадрах
- ✅ **`format_time()`**
  - Читаемое отображение времени (HH:MM:SS)
- ✅ **Автооткрытие карты**
  - `webbrowser.open()` после генерации
- ✅ **Кастомные стили**
  - Профессиональный dark theme

**Результат:** Готовый к использованию инструмент

---

## 📈 Эволюция функционала

```mermaid
gantt
    title Этапы разработки проекта
    dateFormat X
    axisFormat %s
    
    section Прототип
    Базовая детекция + SRT     :0, 1
    
    section Геотеги
    EXIF интеграция            :1, 2
    
    section Визуализация
    Folium карта               :2, 3
    
    section Расширение
    Режим фотографий           :3, 4
    
    section UX
    GUI на PySide6             :4, 5
    
    section Продакшн
    Отчёты + логирование       :5, 6
    
    section Полировка
    Финальные штрихи           :6, 7
```

---

## 🎨 Ключевые особенности архитектуры

| Компонент | Технология | Назначение |
|-----------|------------|------------|
| 🎬 **Видео-процессинг** | OpenCV | Извлечение кадров из MP4/AVI/MOV |
| 🛰️ **Телеметрия** | SRT Parser | GPS из субтитров дронов |
| 🤖 **Детекция** | YOLO | Поиск людей с настраиваемыми порогами |
| 🌍 **Геотеги** | piexif | Встраивание GPS в EXIF |
| 🗺️ **Картография** | Folium | Интерактивные карты с кластеризацией |
| 📊 **Отчёты** | pandas | Excel с метаданными |
| 🖥️ **Интерфейс** | PySide6 | Нативный GUI с тёмной темой |
| 📝 **Логирование** | logging | Полный аудит всех операций |
| 🔐 **Встраивание** | base64 | Автономные HTML-карты |

---

## 🔄 Поток данных

```
📹 Видео/Фото
    ↓
🌍 GPS (SRT/EXIF)
    ↓
🤖 YOLO Детекция
    ↓
🎨 Визуализация (bbox)
    ↓
💾 Сохранение с геотегами
    ↓
┌─────────────┬────────────┐
│             │            │
📊 Excel      🗺️ Folium    📄 Логи
(только люди) (все/люди)  (аудит)
```

---

## 🎯 Итоговая функциональность

### ✅ Что умеет приложение

1. **📹 Обработка видео**
   - Извлечение кадров с настраиваемым шагом (1/5/10/20)
   - Восстановление GPS из SRT-телеметрии

2. **📸 Обработка фотографий**
   - Пакетная обработка папок
   - Чтение GPS из существующих EXIF-тегов

3. **🤖 Детекция людей**
   - Поддержка пользовательских YOLO-моделей
   - Настройка confidence и IoU порогов
   - Подсчёт количества людей на кадре

4. **🗺️ Интерактивная карта**
   - Спутниковый слой Esri WorldImagery
   - Кластеризация маркеров
   - Превью фотографий в popup
   - Метаданные в tooltip

5. **📊 Отчётность**
   - Excel с координатами и временем
   - Логи всех операций
   - Прогресс обработки в реальном времени

6. **🎨 Визуализация**
   - Красные bounding boxes на кадрах
   - Confidence scores над обнаружениями
   - Сохранение обработанных изображений

7. **🔧 Удобство**
   - Сохранение путей между запусками
   - Обработка ошибок с автоисправлением
   - Автооткрытие результатов в браузере


```mermaid
graph TD
    A[Действие пользователя] --> B{Тип действия}
    
    B --> C[Нажал кнопку без модели YOLO]
    B --> D[Видео-режим без SRT]
    B --> E[Папка пуста/без фото]
    B --> F[Нет прав на запись]
    B --> G[Excel занят]
    B --> H[Длинное видео]
    B --> I[Шаг = 20]
    B --> J[Чекбокс все фото]
    B --> K[Своя модель YOLO]
    
    C --> C1[QMessageBox:<br/>«Выберите файл модели YOLO»]
    D --> D1[QMessageBox:<br/>«Выберите MP4/AVI/MOV и SRT»]
    E --> E1[QMessageBox:<br/>«В папке не найдены JPG/PNG»]
    F --> F1[PermissionError →<br/>лог + сообщение]
    G --> G1[Автосохранение:<br/>frames_metadata_дата.xlsx]
    H --> H1[Прогресс-бар + processEvents<br/>UI не зависает]
    I --> I1[Обработка каждого 20-го кадра<br/>Ускорение в 20 раз]
    J --> J1[На карту попадают ВСЕ фото<br/>с геотегом для анализа покрытия]
    K --> K1[Работает без изменений<br/>Агностик к .pt файлу]
    
    style A fill:#e1f5fe
    style B fill:#f3e5f5
    style C1 fill:#ffebee
    style D1 fill:#ffebee
    style E1 fill:#ffebee
    style F1 fill:#fff3e0
    style G1 fill:#e8f5e8
    style H1 fill:#e8f5e8
    style I1 fill:#e3f2fd
    style J1 fill:#e8f5e8
    style K1 fill:#e8f5e8
```

```mermaid
graph LR
    subgraph "Обработка пользовательских действий"
        A1[Ошибки ввода] --> A2[Предупреждения]
        B1[Проблемы доступа] --> B2[Обработка ошибок]
        C1[Настройки] --> C2[Оптимизация]
        D1[Расширения] --> D2[Гибкость]
    end
    
    A2 --> A3[QMessageBox]
    B2 --> B3[Логирование +<br/>альтернативные действия]
    C2 --> C3[Ускорение обработки<br/>Стабильность UI]
    D2 --> D3[Поддержка кастомных<br/>моделей YOLO]
    
    style A1 fill:#ffebee
    style B1 fill:#fff3e0
    style C1 fill:#e3f2fd
    style D1 fill:#e8f5e8
    style A3 fill:#ffcdd2
    style B3 fill:#ffcc80
    style C3 fill:#90caf9
    style D3 fill:#a5d6a7
```
